...
def recommend_songs(req: RecommendSongsRequest, credentials: HTTPAuthorizationCredentials | None = Depends(bearer_scheme)):
    ...
    for attempt in range(5):
        ...
        if req.novelty_mode == "graceful" and attempt >= 2:
        ...
        if has_signature_collision(user_client, req.user_id, signature):
            continue
        if req.novelty_mode == "strict" and req.exclude_previously_shown and overlap > 0:
            continue
        if req.novelty_mode == "graceful" and overlap > req.max_allowed_overlap:
            continue
